...
use crate::task::{TaskContext, get_current_task};
...
impl KernelStack {
    ...
    pub fn push_context(&self, trap_cx: TrapContext, task_cx: TaskContext) -> &'static mut TaskContext {
        unsafe {
            ...
            *trap_cx_ptr = trap_cx;
            let task_cx_ptr = (trap_cx_ptr as usize - size_of::<TaskContext>()) as *mut TaskContext;
            *task_cx_ptr = task_cx;
...
pub fn load_apps() {
    ...
    for i in 0..num_app {
        ...
        let dst = unsafe {
            from_raw_parts_mut(base_i as *mut u8, src.len())
        };
